...
class Eintragen_model extends CI_Model {
		...
		public function gen_user_chart($user_id = NULL)
		{
			...
			if ($user_id) {
				...
				$query = $this->db->query("SELECT t.Vorname as Vorname, t.Name as Name, Teilnehmer_id, (sum(f.Km_zur_Arbeit)+sum(f.Km_Privat)) as Km_Gesamt FROM `fahrtenbuch` f, teilnehmer t where t.id=f.Teilnehmer_id and year(datum) = 2017 GROUP BY Teilnehmer_id ORDER BY Km_Gesamt DESC");
				$ret = '';
				$cnt = 0;
				if ($query->num_rows() > 0) {
			...
			if ($group_id){
				...
				if(is_numeric($group_id)) {
					$query = $this->db->query("SELECT SUM(f.`Km_zur_Arbeit`) AS Km_Arbeit_sum, SUM(f.`Km_Privat`) AS Km_Privat_sum, SUM(f.`Km_zur_Arbeit`)+SUM(f.`Km_Privat`) AS Km_ges_sum ,te.Name, t.Vorname, t.Name
						...
						WHERE Team_id in ($group_id) and year(datum) = 2017
					...
					$query = $this->db->query("SELECT SUM(f.`Km_zur_Arbeit`) AS Km_Arbeit_sum, SUM(f.`Km_Privat`) AS Km_Privat_sum, SUM(f.`Km_zur_Arbeit`)+SUM(f.`Km_Privat`) AS Km_ges_sum, t.Vorname, t.Name
						FROM teilnehmer t JOIN fahrtenbuch f ON (t.id = f.`Teilnehmer_id`)
						WHERE substring_index(Abteilung,' / ',1) = '$group_id' and year(datum) = 2017
